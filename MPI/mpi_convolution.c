...
int main(int argc, char** argv){
    ...
    for (n=0; n<reps; n++){
        ...
        //--Send Requests
        offset = (local_sizes[1]+2)*pixel_size + pixel_size;
        ...
        if((n+1)%50==0){
          offset = (local_sizes[1]+2)*pixel_size;
            for(int i=1*offset + pixel_size ; i<local_sizes[0]*offset + pixel_size*(local_sizes[1]+1) ; i++){
    ...
    char* conv_info = malloc(15*sizeof(char));
    snprintf (conv_info, 15, "_p%d_x%d.raw", numof_processes, reps);
    char* new_image_name = malloc(strlen(image_name)+15);
    memcpy(new_image_name, image_name, strlen(image_name)-4+1);
    snprintf(new_image_name+strlen(image_name)-4, 15, "%s", conv_info);
    ...
    MPI_Reduce(&n, &global_reps, 1, MPI_INT, MPI_MAX, 0, grid_comm);
    if(my_rank==0){
      FILE * results;
      results = fopen("MPI_Convolution_Results.txt", "a");
      fprintf(results, "------------------------------------------\n");
