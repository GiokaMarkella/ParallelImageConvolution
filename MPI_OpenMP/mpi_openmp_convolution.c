...
// compile: mpicc -openmp -o MPI_OpenMP/mpi_openmp_convolution MPI_OpenMP/mpi_openmp_convolution.c
// execute: mpirun -np 4 ./MPI_OpenMP/mpi_openmp_convolution Images/waterfall_grey_1920_2520.raw grey 1920 2520 10
...
#ifdef _OPENMP
#include <omp.h>
#define THREDNUM 9 //number of threads of each process for taking care of serial calculations
#endif
...
static inline void Convolute_Block(unsigned char *, unsigned char *, norm_filter_t, int, int, int, int, int);
static inline void Convolute_Pixel(unsigned char *, unsigned char *, norm_filter_t, int, int, int, int);

static inline  int Check_Arguments(char** argv, int argc, colour_model *, int *, int *, int *);
static inline void Set_Filter(int, filter_type_t, norm_filter_t*);
static inline int Find_Neighbour(MPI_Comm *, int *, int *, int *);
...
int main(int argc, char** argv){
    ...
    //--Convolution Loop
    int diff; /*for convergence check*/
    for (int n=0; n<reps; n++){
        ...
        offset = (local_sizes[1]+2)*2*pixel_size;
        Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter,2,local_sizes[0]-2,local_sizes[1]+2, local_sizes[1]-2, pixel_size);
        ...
        while(1){
            ...
            else if(index==0){/*up row*/
              offset = ((local_sizes[1]+2)*pixel_size);
              Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, 2, 1, local_sizes[1]+2, local_sizes[1]-2, pixel_size);
            }
            else if(index==3){/*down row*/
              offset = ((local_sizes[1]+2)*pixel_size)*local_sizes[0];
              Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, 2, 1, local_sizes[1]+2, local_sizes[1]-2, pixel_size);
            }
            /*left or right border*/
            else if(index==1){/*left column*/
              offset = (local_sizes[1]+2)*pixel_size*2;
              Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, 1, local_sizes[0]-2, local_sizes[1]+2, 1, pixel_size);
            }
            else if(index==2){/*right column*/
              offset = (local_sizes[1]+2)*pixel_size*2;
              Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, local_sizes[1], local_sizes[0]-2, local_sizes[1]+2, 1, pixel_size);
        ...
        while(1){
            ...
            /*convolute what got received*/
            if(index==MPI_UNDEFINED) break;
            else if(index==0){/*top-left*/
                offset = (local_sizes[1]+2)*pixel_size;
                Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, 1, 1, local_sizes[1]+2, 1, pixel_size);
            }
            else if(index==1){/*top-right*/
                offset = (local_sizes[1]+2)*pixel_size;
                Convolute_Block(&local_buf[offset], &temp_buf[offset], h_filter, local_sizes[1], 1, local_sizes[1]+2, 1, pixel_size);
            }
    ...
    //--Make and store new (convoluted) image file
    char* conv_info = malloc(19*sizeof(char));
    snprintf (conv_info, 19, "_p%d_x%d_omp.raw", numof_processes, reps);
    char* new_image_name = malloc(strlen(image_name)+19);
    memcpy(new_image_name, image_name, strlen(image_name)-4+1);
    snprintf(new_image_name+strlen(image_name)-4, 19, "%s", conv_info);
    ...
    MPI_Reduce(&reps, &global_reps, 1, MPI_INT, MPI_MAX, 0, grid_comm);
    if(my_rank==0){
      FILE * results;
      results = fopen("MPI+OMP_Convolution_Results.txt", "a");
      fprintf(results, "------------------------------------------\n");
      fprintf(results, "--Convolution arguments:\n");
      fprintf(results, "\tImage - %s\n", image_name);
      if(image_type==RGB)fprintf(results, "\tColour Model - RGB\n");
      else if(image_type==GREY)fprintf(results, "\tColour Model - GREY\n");
      fprintf(results, "\tDimensions - %d x %d\n", image_width, image_height);
      fprintf(results, "\tRepetitions - %d\n", reps);
      fprintf(results, "\tProcesses - %d\n",numof_processes );
      fprintf(results, "--Convolution results:\n");
      fprintf(results, "\tElapsed time %f\n", global_time);
      fprintf(results, "\tConvolution stopped after -%d- repetitions\n", global_reps);
      fclose(results);
    }
